    ...
    public class Error
    {
        public static string String(FMOD.RESULT errcode)
        {
            switch (errcode)
            {
                ...
                case FMOD.RESULT.ERR_DSP_CONNECTION:            return "DSP connection error.  Connection possibly caused a cyclic dependancy.  Or tried to connect a tree too many units deep (more than 128). ";
                ...
                case FMOD.RESULT.ERR_EVENT_ALREADY_LOADED:      return "The specified project or bank has already been loaded. Having multiple copies of the same project loaded simultaneously is forbidden. ";
                ...
                case FMOD.RESULT.ERR_EVENT_MAXSTREAMS:          return "Event failed because 'Max streams' was hit when FMOD.RESULT.EVENT_INIT_FAIL_ON_MAXSTREAMS was specified. ";
                case FMOD.RESULT.ERR_EVENT_MISMATCH:            return "FSB mismatches the FEV it was compiled with, the stream/sample mode it was meant to be created with was different, or the FEV was built for a different platform. ";
                ...
                case FMOD.RESULT.ERR_EVENT_WONT_STOP:           return "The event cannot be released because it will not terminate, call stop to allow releasing of this event ";
                ...
                case FMOD.RESULT.ERR_MEMORY_CANTPOINT:          return "Can't use FMOD.RESULT.OPENMEMORY_POINT on non PCM source data, or non mp3/xma/adpcm data if FMOD.RESULT.CREATECOMPRESSEDSAMPLE was used. ";
                ...
                case FMOD.RESULT.ERR_NEEDSHARDWARE:             return "Tried to use a feature that requires hardware support.  (ie trying to play a GCADPCM compressed sound in software on Wii). ";
                ...
                case FMOD.RESULT.ERR_OUTPUT_NOHARDWARE:         return "FMOD.RESULT.HARDWARE was specified but the sound card does not have the resources necessary to play it. ";
                ...
                case FMOD.RESULT.ERR_REVERB_INSTANCE:           return "Specified instance in FMOD.RESULT.REVERB_PROPERTIES couldn't be set. Most likely because it is an invalid instance number or the reverb doesnt exist. ";
                case FMOD.RESULT.ERR_STUDIO_UNINITIALIZED:      return "The Studio::System object is not yet initialized ";
                case FMOD.RESULT.ERR_SUBSOUNDS:                 return "The error occured because the sound referenced contains subsounds when it shouldn't have, or it doesn't contain subsounds when it should have.  The operation may also not be able to be performed on a parent sound. ";
